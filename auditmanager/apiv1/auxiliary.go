...
func (op *GenerateAuditReportOperation) Wait(ctx context.Context, opts ...gax.CallOption) (*auditmanagerpb.AuditReport, error) {
...
func (op *GenerateAuditReportOperation) Poll(ctx context.Context, opts ...gax.CallOption) (*auditmanagerpb.AuditReport, error) {
	opts = append([]gax.CallOption{gax.WithPath(op.pollPath)}, opts...)
	var resp auditmanagerpb.AuditReport
...
func (op *GenerateAuditReportOperation) Metadata() (*auditmanagerpb.OperationMetadata, error) {
	var meta auditmanagerpb.OperationMetadata
...
func (op *GenerateAuditReportOperation) Done() bool {
	return op.lro.Done()
}

// Name returns the name of the long-running operation.
// The name is assigned by the server and is unique within the service from which the operation is created.
func (op *GenerateAuditReportOperation) Name() string {
	return op.lro.Name()
}
...
func (it *AuditReportIterator) Next() (*auditmanagerpb.AuditReport, error) {
	var item *auditmanagerpb.AuditReport
...
func (it *AuditReportIterator) bufLen() int {
	return len(it.items)
}

func (it *AuditReportIterator) takeBuf() interface{} {
	b := it.items
	it.items = nil
	return b
}
...
// Next returns the next result. Its second return value is iterator.Done if there are no more
// results. Once Next returns Done, all subsequent calls will return Done.
func (it *ControlIterator) Next() (*auditmanagerpb.Control, error) {
	var item *auditmanagerpb.Control
	if err := it.nextFunc(); err != nil {
		return item, err
	}
	item = it.items[0]
...
type ResourceEnrollmentStatusIterator struct {
	items    []*auditmanagerpb.ResourceEnrollmentStatus
	pageInfo *iterator.PageInfo
	nextFunc func() error
...
// Next returns the next result. Its second return value is iterator.Done if there are no more
// results. Once Next returns Done, all subsequent calls will return Done.
func (it *ResourceEnrollmentStatusIterator) Next() (*auditmanagerpb.ResourceEnrollmentStatus, error) {
	var item *auditmanagerpb.ResourceEnrollmentStatus
	if err := it.nextFunc(); err != nil {
		return item, err
	}
	item = it.items[0]
...
func (it *ResourceEnrollmentStatusIterator) bufLen() int {
	return len(it.items)
}

func (it *ResourceEnrollmentStatusIterator) takeBuf() interface{} {
	b := it.items
	it.items = nil
	return b
}
