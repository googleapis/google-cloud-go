...
func (ap *arrowParser) convertArrowRows(recordBatch *bqStoragepb.ArrowRecordBatch) ([][]Value, error) {
	var rs [][]Value
	...
	for r.Next() {
		rec := r.Record()
		tmp := make([][]Value, rec.NumRows())
		for i := range tmp {
			tmp[i] = make([]Value, rec.NumCols())
		}
...
func convertArrowValue(col arrow.Array, i int, ft arrow.DataType, fs *FieldSchema) (Value, error) {
	...
	case *arrow.StringType:
		...
		return convertValue(fmt.Sprintf("%v", arr.Value(i)), fs.Type, fs.Schema)
	case *arrow.Int8Type:
		arr := col.(*array.Int8)
		return convertValue(fmt.Sprintf("%v", arr.Value(i)), fs.Type, fs.Schema)
	...
	case *arrow.Date32Type:
		...
		return convertValue(arr.Value(i).FormattedString(), fs.Type, fs.Schema)
