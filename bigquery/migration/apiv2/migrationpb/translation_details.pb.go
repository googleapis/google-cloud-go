// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.33.2
// source: google/cloud/bigquery/migration/v2/translation_details.proto

package migrationpb

import (
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"

	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Suggestion type.
type SuggestionStep_SuggestionType int32

const (
	// Suggestion type unspecified.
	SuggestionStep_SUGGESTION_TYPE_UNSPECIFIED SuggestionStep_SuggestionType = 0
	// Query customization.
	SuggestionStep_QUERY_CUSTOMIZATION SuggestionStep_SuggestionType = 1
	// Translation explanation.
	SuggestionStep_TRANSLATION_EXPLANATION SuggestionStep_SuggestionType = 2
)

// Enum value maps for SuggestionStep_SuggestionType.
var (
	SuggestionStep_SuggestionType_name = map[int32]string{
		0: "SUGGESTION_TYPE_UNSPECIFIED",
		1: "QUERY_CUSTOMIZATION",
		2: "TRANSLATION_EXPLANATION",
	}
	SuggestionStep_SuggestionType_value = map[string]int32{
		"SUGGESTION_TYPE_UNSPECIFIED": 0,
		"QUERY_CUSTOMIZATION":         1,
		"TRANSLATION_EXPLANATION":     2,
	}
)

func (x SuggestionStep_SuggestionType) Enum() *SuggestionStep_SuggestionType {
	p := new(SuggestionStep_SuggestionType)
	*p = x
	return p
}

func (x SuggestionStep_SuggestionType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SuggestionStep_SuggestionType) Descriptor() protoreflect.EnumDescriptor {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_enumTypes[0].Descriptor()
}

func (SuggestionStep_SuggestionType) Type() protoreflect.EnumType {
	return &file_google_cloud_bigquery_migration_v2_translation_details_proto_enumTypes[0]
}

func (x SuggestionStep_SuggestionType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SuggestionStep_SuggestionType.Descriptor instead.
func (SuggestionStep_SuggestionType) EnumDescriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{2, 0}
}

// The target to apply the suggestion to.
type SuggestionStep_RewriteTarget int32

const (
	// Rewrite target unspecified.
	SuggestionStep_REWRITE_TARGET_UNSPECIFIED SuggestionStep_RewriteTarget = 0
	// Source SQL.
	SuggestionStep_SOURCE_SQL SuggestionStep_RewriteTarget = 1
	// Target SQL.
	SuggestionStep_TARGET_SQL SuggestionStep_RewriteTarget = 2
)

// Enum value maps for SuggestionStep_RewriteTarget.
var (
	SuggestionStep_RewriteTarget_name = map[int32]string{
		0: "REWRITE_TARGET_UNSPECIFIED",
		1: "SOURCE_SQL",
		2: "TARGET_SQL",
	}
	SuggestionStep_RewriteTarget_value = map[string]int32{
		"REWRITE_TARGET_UNSPECIFIED": 0,
		"SOURCE_SQL":                 1,
		"TARGET_SQL":                 2,
	}
)

func (x SuggestionStep_RewriteTarget) Enum() *SuggestionStep_RewriteTarget {
	p := new(SuggestionStep_RewriteTarget)
	*p = x
	return p
}

func (x SuggestionStep_RewriteTarget) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SuggestionStep_RewriteTarget) Descriptor() protoreflect.EnumDescriptor {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_enumTypes[1].Descriptor()
}

func (SuggestionStep_RewriteTarget) Type() protoreflect.EnumType {
	return &file_google_cloud_bigquery_migration_v2_translation_details_proto_enumTypes[1]
}

func (x SuggestionStep_RewriteTarget) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SuggestionStep_RewriteTarget.Descriptor instead.
func (SuggestionStep_RewriteTarget) EnumDescriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{2, 1}
}

// The translation details to capture the necessary settings for a translation
// job.
type TranslationDetails struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The mapping from source to target SQL.
	SourceTargetMapping []*SourceTargetMapping `protobuf:"bytes,1,rep,name=source_target_mapping,json=sourceTargetMapping,proto3" json:"source_target_mapping,omitempty"`
	// The base URI for all writes to persistent storage.
	TargetBaseUri string `protobuf:"bytes,2,opt,name=target_base_uri,json=targetBaseUri,proto3" json:"target_base_uri,omitempty"`
	// The default source environment values for the translation.
	SourceEnvironment *SourceEnvironment `protobuf:"bytes,3,opt,name=source_environment,json=sourceEnvironment,proto3" json:"source_environment,omitempty"`
	// The list of literal targets that will be directly returned to the response.
	// Each entry consists of the constructed path, EXCLUDING the base path. Not
	// providing a target_base_uri will prevent writing to persistent storage.
	TargetReturnLiterals []string `protobuf:"bytes,4,rep,name=target_return_literals,json=targetReturnLiterals,proto3" json:"target_return_literals,omitempty"`
	// The types of output to generate, e.g. sql, metadata,
	// lineage_from_sql_scripts, etc. If not specified, a default set of
	// targets will be generated. Some additional target types may be slower to
	// generate. See the documentation for the set of available target types.
	TargetTypes []string `protobuf:"bytes,5,rep,name=target_types,json=targetTypes,proto3" json:"target_types,omitempty"`
	// The configuration for the suggestion if requested as a target type.
	SuggestionConfig *SuggestionConfig `protobuf:"bytes,6,opt,name=suggestion_config,json=suggestionConfig,proto3" json:"suggestion_config,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *TranslationDetails) Reset() {
	*x = TranslationDetails{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TranslationDetails) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TranslationDetails) ProtoMessage() {}

func (x *TranslationDetails) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TranslationDetails.ProtoReflect.Descriptor instead.
func (*TranslationDetails) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{0}
}

func (x *TranslationDetails) GetSourceTargetMapping() []*SourceTargetMapping {
	if x != nil {
		return x.SourceTargetMapping
	}
	return nil
}

func (x *TranslationDetails) GetTargetBaseUri() string {
	if x != nil {
		return x.TargetBaseUri
	}
	return ""
}

func (x *TranslationDetails) GetSourceEnvironment() *SourceEnvironment {
	if x != nil {
		return x.SourceEnvironment
	}
	return nil
}

func (x *TranslationDetails) GetTargetReturnLiterals() []string {
	if x != nil {
		return x.TargetReturnLiterals
	}
	return nil
}

func (x *TranslationDetails) GetTargetTypes() []string {
	if x != nil {
		return x.TargetTypes
	}
	return nil
}

func (x *TranslationDetails) GetSuggestionConfig() *SuggestionConfig {
	if x != nil {
		return x.SuggestionConfig
	}
	return nil
}

// The configuration for the suggestion if requested as a target type.
type SuggestionConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The list of suggestion steps to skip.
	SkipSuggestionSteps []*SuggestionStep `protobuf:"bytes,1,rep,name=skip_suggestion_steps,json=skipSuggestionSteps,proto3" json:"skip_suggestion_steps,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *SuggestionConfig) Reset() {
	*x = SuggestionConfig{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SuggestionConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SuggestionConfig) ProtoMessage() {}

func (x *SuggestionConfig) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SuggestionConfig.ProtoReflect.Descriptor instead.
func (*SuggestionConfig) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{1}
}

func (x *SuggestionConfig) GetSkipSuggestionSteps() []*SuggestionStep {
	if x != nil {
		return x.SkipSuggestionSteps
	}
	return nil
}

// Suggestion step to skip.
type SuggestionStep struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The type of suggestion.
	SuggestionType SuggestionStep_SuggestionType `protobuf:"varint,1,opt,name=suggestion_type,json=suggestionType,proto3,enum=google.cloud.bigquery.migration.v2.SuggestionStep_SuggestionType" json:"suggestion_type,omitempty"`
	// The rewrite target.
	RewriteTarget SuggestionStep_RewriteTarget `protobuf:"varint,2,opt,name=rewrite_target,json=rewriteTarget,proto3,enum=google.cloud.bigquery.migration.v2.SuggestionStep_RewriteTarget" json:"rewrite_target,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SuggestionStep) Reset() {
	*x = SuggestionStep{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SuggestionStep) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SuggestionStep) ProtoMessage() {}

func (x *SuggestionStep) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SuggestionStep.ProtoReflect.Descriptor instead.
func (*SuggestionStep) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{2}
}

func (x *SuggestionStep) GetSuggestionType() SuggestionStep_SuggestionType {
	if x != nil {
		return x.SuggestionType
	}
	return SuggestionStep_SUGGESTION_TYPE_UNSPECIFIED
}

func (x *SuggestionStep) GetRewriteTarget() SuggestionStep_RewriteTarget {
	if x != nil {
		return x.RewriteTarget
	}
	return SuggestionStep_REWRITE_TARGET_UNSPECIFIED
}

// Represents one mapping from a source SQL to a target SQL.
type SourceTargetMapping struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The source SQL or the path to it.
	SourceSpec *SourceSpec `protobuf:"bytes,1,opt,name=source_spec,json=sourceSpec,proto3" json:"source_spec,omitempty"`
	// The target SQL or the path for it.
	TargetSpec    *TargetSpec `protobuf:"bytes,2,opt,name=target_spec,json=targetSpec,proto3" json:"target_spec,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SourceTargetMapping) Reset() {
	*x = SourceTargetMapping{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SourceTargetMapping) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SourceTargetMapping) ProtoMessage() {}

func (x *SourceTargetMapping) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SourceTargetMapping.ProtoReflect.Descriptor instead.
func (*SourceTargetMapping) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{3}
}

func (x *SourceTargetMapping) GetSourceSpec() *SourceSpec {
	if x != nil {
		return x.SourceSpec
	}
	return nil
}

func (x *SourceTargetMapping) GetTargetSpec() *TargetSpec {
	if x != nil {
		return x.TargetSpec
	}
	return nil
}

// Represents one path to the location that holds source data.
type SourceSpec struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The specific source SQL.
	//
	// Types that are valid to be assigned to Source:
	//
	//	*SourceSpec_BaseUri
	//	*SourceSpec_Literal
	//	*SourceSpec_GcsFilePath
	Source isSourceSpec_Source `protobuf_oneof:"source"`
	// Optional. The optional field to specify the encoding of the sql bytes.
	Encoding      string `protobuf:"bytes,3,opt,name=encoding,proto3" json:"encoding,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SourceSpec) Reset() {
	*x = SourceSpec{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SourceSpec) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SourceSpec) ProtoMessage() {}

func (x *SourceSpec) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SourceSpec.ProtoReflect.Descriptor instead.
func (*SourceSpec) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{4}
}

func (x *SourceSpec) GetSource() isSourceSpec_Source {
	if x != nil {
		return x.Source
	}
	return nil
}

func (x *SourceSpec) GetBaseUri() string {
	if x != nil {
		if x, ok := x.Source.(*SourceSpec_BaseUri); ok {
			return x.BaseUri
		}
	}
	return ""
}

func (x *SourceSpec) GetLiteral() *Literal {
	if x != nil {
		if x, ok := x.Source.(*SourceSpec_Literal); ok {
			return x.Literal
		}
	}
	return nil
}

func (x *SourceSpec) GetGcsFilePath() string {
	if x != nil {
		if x, ok := x.Source.(*SourceSpec_GcsFilePath); ok {
			return x.GcsFilePath
		}
	}
	return ""
}

func (x *SourceSpec) GetEncoding() string {
	if x != nil {
		return x.Encoding
	}
	return ""
}

type isSourceSpec_Source interface {
	isSourceSpec_Source()
}

type SourceSpec_BaseUri struct {
	// The base URI for all files to be read in as sources for translation.
	BaseUri string `protobuf:"bytes,1,opt,name=base_uri,json=baseUri,proto3,oneof"`
}

type SourceSpec_Literal struct {
	// Source literal.
	Literal *Literal `protobuf:"bytes,2,opt,name=literal,proto3,oneof"`
}

type SourceSpec_GcsFilePath struct {
	// The path to a single source file in Cloud Storage.
	GcsFilePath string `protobuf:"bytes,4,opt,name=gcs_file_path,json=gcsFilePath,proto3,oneof"`
}

func (*SourceSpec_BaseUri) isSourceSpec_Source() {}

func (*SourceSpec_Literal) isSourceSpec_Source() {}

func (*SourceSpec_GcsFilePath) isSourceSpec_Source() {}

// Represents one path to the location that holds target data.
type TargetSpec struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The relative path for the target data. Given source file
	// `base_uri/input/sql`, the output would be
	// `target_base_uri/sql/relative_path/input.sql`.
	RelativePath  string `protobuf:"bytes,1,opt,name=relative_path,json=relativePath,proto3" json:"relative_path,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TargetSpec) Reset() {
	*x = TargetSpec{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TargetSpec) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TargetSpec) ProtoMessage() {}

func (x *TargetSpec) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TargetSpec.ProtoReflect.Descriptor instead.
func (*TargetSpec) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{5}
}

func (x *TargetSpec) GetRelativePath() string {
	if x != nil {
		return x.RelativePath
	}
	return ""
}

// Literal data.
type Literal struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The literal SQL contents.
	//
	// Types that are valid to be assigned to LiteralData:
	//
	//	*Literal_LiteralString
	//	*Literal_LiteralBytes
	LiteralData isLiteral_LiteralData `protobuf_oneof:"literal_data"`
	// Required. The identifier of the literal entry.
	RelativePath  string `protobuf:"bytes,1,opt,name=relative_path,json=relativePath,proto3" json:"relative_path,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Literal) Reset() {
	*x = Literal{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Literal) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Literal) ProtoMessage() {}

func (x *Literal) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Literal.ProtoReflect.Descriptor instead.
func (*Literal) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{6}
}

func (x *Literal) GetLiteralData() isLiteral_LiteralData {
	if x != nil {
		return x.LiteralData
	}
	return nil
}

func (x *Literal) GetLiteralString() string {
	if x != nil {
		if x, ok := x.LiteralData.(*Literal_LiteralString); ok {
			return x.LiteralString
		}
	}
	return ""
}

func (x *Literal) GetLiteralBytes() []byte {
	if x != nil {
		if x, ok := x.LiteralData.(*Literal_LiteralBytes); ok {
			return x.LiteralBytes
		}
	}
	return nil
}

func (x *Literal) GetRelativePath() string {
	if x != nil {
		return x.RelativePath
	}
	return ""
}

type isLiteral_LiteralData interface {
	isLiteral_LiteralData()
}

type Literal_LiteralString struct {
	// Literal string data.
	LiteralString string `protobuf:"bytes,2,opt,name=literal_string,json=literalString,proto3,oneof"`
}

type Literal_LiteralBytes struct {
	// Literal byte data.
	LiteralBytes []byte `protobuf:"bytes,3,opt,name=literal_bytes,json=literalBytes,proto3,oneof"`
}

func (*Literal_LiteralString) isLiteral_LiteralData() {}

func (*Literal_LiteralBytes) isLiteral_LiteralData() {}

// Represents the default source environment values for the translation.
type SourceEnvironment struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The default database name to fully qualify SQL objects when their database
	// name is missing.
	DefaultDatabase string `protobuf:"bytes,1,opt,name=default_database,json=defaultDatabase,proto3" json:"default_database,omitempty"`
	// The schema search path. When SQL objects are missing schema name,
	// translation engine will search through this list to find the value.
	SchemaSearchPath []string `protobuf:"bytes,2,rep,name=schema_search_path,json=schemaSearchPath,proto3" json:"schema_search_path,omitempty"`
	// Optional. Expects a validQ BigQuery dataset ID that exists, e.g.,
	// project-123.metadata_store_123.  If specified, translation will search and
	// read the required schema information from a metadata store in this dataset.
	// If metadata store doesn't exist, translation will parse the metadata file
	// and upload the schema info to a temp table in the dataset to speed up
	// future translation jobs.
	MetadataStoreDataset string `protobuf:"bytes,3,opt,name=metadata_store_dataset,json=metadataStoreDataset,proto3" json:"metadata_store_dataset,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *SourceEnvironment) Reset() {
	*x = SourceEnvironment{}
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SourceEnvironment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SourceEnvironment) ProtoMessage() {}

func (x *SourceEnvironment) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SourceEnvironment.ProtoReflect.Descriptor instead.
func (*SourceEnvironment) Descriptor() ([]byte, []int) {
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP(), []int{7}
}

func (x *SourceEnvironment) GetDefaultDatabase() string {
	if x != nil {
		return x.DefaultDatabase
	}
	return ""
}

func (x *SourceEnvironment) GetSchemaSearchPath() []string {
	if x != nil {
		return x.SchemaSearchPath
	}
	return nil
}

func (x *SourceEnvironment) GetMetadataStoreDataset() string {
	if x != nil {
		return x.MetadataStoreDataset
	}
	return ""
}

var File_google_cloud_bigquery_migration_v2_translation_details_proto protoreflect.FileDescriptor

const file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDesc = "" +
	"\n" +
	"<google/cloud/bigquery/migration/v2/translation_details.proto\x12\"google.cloud.bigquery.migration.v2\x1a\x1fgoogle/api/field_behavior.proto\"\xcb\x03\n" +
	"\x12TranslationDetails\x12k\n" +
	"\x15source_target_mapping\x18\x01 \x03(\v27.google.cloud.bigquery.migration.v2.SourceTargetMappingR\x13sourceTargetMapping\x12&\n" +
	"\x0ftarget_base_uri\x18\x02 \x01(\tR\rtargetBaseUri\x12d\n" +
	"\x12source_environment\x18\x03 \x01(\v25.google.cloud.bigquery.migration.v2.SourceEnvironmentR\x11sourceEnvironment\x124\n" +
	"\x16target_return_literals\x18\x04 \x03(\tR\x14targetReturnLiterals\x12!\n" +
	"\ftarget_types\x18\x05 \x03(\tR\vtargetTypes\x12a\n" +
	"\x11suggestion_config\x18\x06 \x01(\v24.google.cloud.bigquery.migration.v2.SuggestionConfigR\x10suggestionConfig\"z\n" +
	"\x10SuggestionConfig\x12f\n" +
	"\x15skip_suggestion_steps\x18\x01 \x03(\v22.google.cloud.bigquery.migration.v2.SuggestionStepR\x13skipSuggestionSteps\"\x9f\x03\n" +
	"\x0eSuggestionStep\x12j\n" +
	"\x0fsuggestion_type\x18\x01 \x01(\x0e2A.google.cloud.bigquery.migration.v2.SuggestionStep.SuggestionTypeR\x0esuggestionType\x12g\n" +
	"\x0erewrite_target\x18\x02 \x01(\x0e2@.google.cloud.bigquery.migration.v2.SuggestionStep.RewriteTargetR\rrewriteTarget\"g\n" +
	"\x0eSuggestionType\x12\x1f\n" +
	"\x1bSUGGESTION_TYPE_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13QUERY_CUSTOMIZATION\x10\x01\x12\x1b\n" +
	"\x17TRANSLATION_EXPLANATION\x10\x02\"O\n" +
	"\rRewriteTarget\x12\x1e\n" +
	"\x1aREWRITE_TARGET_UNSPECIFIED\x10\x00\x12\x0e\n" +
	"\n" +
	"SOURCE_SQL\x10\x01\x12\x0e\n" +
	"\n" +
	"TARGET_SQL\x10\x02\"\xb7\x01\n" +
	"\x13SourceTargetMapping\x12O\n" +
	"\vsource_spec\x18\x01 \x01(\v2..google.cloud.bigquery.migration.v2.SourceSpecR\n" +
	"sourceSpec\x12O\n" +
	"\vtarget_spec\x18\x02 \x01(\v2..google.cloud.bigquery.migration.v2.TargetSpecR\n" +
	"targetSpec\"\xc3\x01\n" +
	"\n" +
	"SourceSpec\x12\x1b\n" +
	"\bbase_uri\x18\x01 \x01(\tH\x00R\abaseUri\x12G\n" +
	"\aliteral\x18\x02 \x01(\v2+.google.cloud.bigquery.migration.v2.LiteralH\x00R\aliteral\x12$\n" +
	"\rgcs_file_path\x18\x04 \x01(\tH\x00R\vgcsFilePath\x12\x1f\n" +
	"\bencoding\x18\x03 \x01(\tB\x03\xe0A\x01R\bencodingB\b\n" +
	"\x06source\"1\n" +
	"\n" +
	"TargetSpec\x12#\n" +
	"\rrelative_path\x18\x01 \x01(\tR\frelativePath\"\x93\x01\n" +
	"\aLiteral\x12'\n" +
	"\x0eliteral_string\x18\x02 \x01(\tH\x00R\rliteralString\x12%\n" +
	"\rliteral_bytes\x18\x03 \x01(\fH\x00R\fliteralBytes\x12(\n" +
	"\rrelative_path\x18\x01 \x01(\tB\x03\xe0A\x02R\frelativePathB\x0e\n" +
	"\fliteral_data\"\xa7\x01\n" +
	"\x11SourceEnvironment\x12)\n" +
	"\x10default_database\x18\x01 \x01(\tR\x0fdefaultDatabase\x12,\n" +
	"\x12schema_search_path\x18\x02 \x03(\tR\x10schemaSearchPath\x129\n" +
	"\x16metadata_store_dataset\x18\x03 \x01(\tB\x03\xe0A\x01R\x14metadataStoreDatasetB\xd3\x01\n" +
	"&com.google.cloud.bigquery.migration.v2B\x17TranslationDetailsProtoP\x01ZDcloud.google.com/go/bigquery/migration/apiv2/migrationpb;migrationpb\xaa\x02\"Google.Cloud.BigQuery.Migration.V2\xca\x02\"Google\\Cloud\\BigQuery\\Migration\\V2b\x06proto3"

var (
	file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescOnce sync.Once
	file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescData []byte
)

func file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescGZIP() []byte {
	file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescOnce.Do(func() {
		file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDesc), len(file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDesc)))
	})
	return file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDescData
}

var file_google_cloud_bigquery_migration_v2_translation_details_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_google_cloud_bigquery_migration_v2_translation_details_proto_goTypes = []any{
	(SuggestionStep_SuggestionType)(0), // 0: google.cloud.bigquery.migration.v2.SuggestionStep.SuggestionType
	(SuggestionStep_RewriteTarget)(0),  // 1: google.cloud.bigquery.migration.v2.SuggestionStep.RewriteTarget
	(*TranslationDetails)(nil),         // 2: google.cloud.bigquery.migration.v2.TranslationDetails
	(*SuggestionConfig)(nil),           // 3: google.cloud.bigquery.migration.v2.SuggestionConfig
	(*SuggestionStep)(nil),             // 4: google.cloud.bigquery.migration.v2.SuggestionStep
	(*SourceTargetMapping)(nil),        // 5: google.cloud.bigquery.migration.v2.SourceTargetMapping
	(*SourceSpec)(nil),                 // 6: google.cloud.bigquery.migration.v2.SourceSpec
	(*TargetSpec)(nil),                 // 7: google.cloud.bigquery.migration.v2.TargetSpec
	(*Literal)(nil),                    // 8: google.cloud.bigquery.migration.v2.Literal
	(*SourceEnvironment)(nil),          // 9: google.cloud.bigquery.migration.v2.SourceEnvironment
}
var file_google_cloud_bigquery_migration_v2_translation_details_proto_depIdxs = []int32{
	5, // 0: google.cloud.bigquery.migration.v2.TranslationDetails.source_target_mapping:type_name -> google.cloud.bigquery.migration.v2.SourceTargetMapping
	9, // 1: google.cloud.bigquery.migration.v2.TranslationDetails.source_environment:type_name -> google.cloud.bigquery.migration.v2.SourceEnvironment
	3, // 2: google.cloud.bigquery.migration.v2.TranslationDetails.suggestion_config:type_name -> google.cloud.bigquery.migration.v2.SuggestionConfig
	4, // 3: google.cloud.bigquery.migration.v2.SuggestionConfig.skip_suggestion_steps:type_name -> google.cloud.bigquery.migration.v2.SuggestionStep
	0, // 4: google.cloud.bigquery.migration.v2.SuggestionStep.suggestion_type:type_name -> google.cloud.bigquery.migration.v2.SuggestionStep.SuggestionType
	1, // 5: google.cloud.bigquery.migration.v2.SuggestionStep.rewrite_target:type_name -> google.cloud.bigquery.migration.v2.SuggestionStep.RewriteTarget
	6, // 6: google.cloud.bigquery.migration.v2.SourceTargetMapping.source_spec:type_name -> google.cloud.bigquery.migration.v2.SourceSpec
	7, // 7: google.cloud.bigquery.migration.v2.SourceTargetMapping.target_spec:type_name -> google.cloud.bigquery.migration.v2.TargetSpec
	8, // 8: google.cloud.bigquery.migration.v2.SourceSpec.literal:type_name -> google.cloud.bigquery.migration.v2.Literal
	9, // [9:9] is the sub-list for method output_type
	9, // [9:9] is the sub-list for method input_type
	9, // [9:9] is the sub-list for extension type_name
	9, // [9:9] is the sub-list for extension extendee
	0, // [0:9] is the sub-list for field type_name
}

func init() { file_google_cloud_bigquery_migration_v2_translation_details_proto_init() }
func file_google_cloud_bigquery_migration_v2_translation_details_proto_init() {
	if File_google_cloud_bigquery_migration_v2_translation_details_proto != nil {
		return
	}
	file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[4].OneofWrappers = []any{
		(*SourceSpec_BaseUri)(nil),
		(*SourceSpec_Literal)(nil),
		(*SourceSpec_GcsFilePath)(nil),
	}
	file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes[6].OneofWrappers = []any{
		(*Literal_LiteralString)(nil),
		(*Literal_LiteralBytes)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDesc), len(file_google_cloud_bigquery_migration_v2_translation_details_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_google_cloud_bigquery_migration_v2_translation_details_proto_goTypes,
		DependencyIndexes: file_google_cloud_bigquery_migration_v2_translation_details_proto_depIdxs,
		EnumInfos:         file_google_cloud_bigquery_migration_v2_translation_details_proto_enumTypes,
		MessageInfos:      file_google_cloud_bigquery_migration_v2_translation_details_proto_msgTypes,
	}.Build()
	File_google_cloud_bigquery_migration_v2_translation_details_proto = out.File
	file_google_cloud_bigquery_migration_v2_translation_details_proto_goTypes = nil
	file_google_cloud_bigquery_migration_v2_translation_details_proto_depIdxs = nil
}
