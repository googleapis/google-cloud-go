...
func (q *partialResultQueue) peekLast() (*internalPartialResultSet, error) {
	if q.empty() {
...
func (q *partialResultQueue) push(r *internalPartialResultSet) {
	if q.q == nil {
		q.q = make([]*internalPartialResultSet, 8 /* arbitrary */)
	}
	if q.n == cap(q.q) {
		buf := make([]*internalPartialResultSet, cap(q.q)*2)
		for i := 0; i < q.n; i++ {
...
func (q *partialResultQueue) pop() *internalPartialResultSet {
	if q.n == 0 {
		return nil
	}
	r := q.q[q.first]
...
func (q *partialResultQueue) dump() []*internalPartialResultSet {
	var dq []*internalPartialResultSet
...
func (d *resumableStreamDecoder) tryRecv(mt *builtinMetricsTracer, retryer gax.Retryer) {
	var res *internalPartialResultSet
...
func (d *resumableStreamDecoder) get() *internalPartialResultSet {
	return d.np
}
...
func (p *partialResultSetDecoder) yield(chunked, last bool) *Row {
	if len(p.row.vals) == len(p.row.fields) && (!chunked || !last) {
		...
		fresh := Row{
			fields: p.row.fields,
			vals:   make([]*internalValue, len(p.row.vals)),
		}
...
func (p *partialResultSetDecoder) add(r *internalPartialResultSet) ([]*Row, *sppb.ResultSetMetadata, error) {
	var rows []*Row
	if r.Metadata != nil {
...
// protobuf Values.
func (p *partialResultSetDecoder) isMergeable(a *internalValue) bool {
	kind := internalValueKindOf(a)
	return kind == internalValueString || kind == internalValueList
}

// errIncompatibleMergeTypes returns error for incompatible protobuf types that
// cannot be merged by partialResultSetDecoder.
func errIncompatibleMergeTypes(a, b *internalValue) error {
	return spannerErrorf(codes.FailedPrecondition, "incompatible type in chunked PartialResultSet. expected (%s), got (%s)", internalValueKindName(a), internalValueKindName(b))
}

// errUnsupportedMergeType returns error for protobuf type that cannot be merged
// to other protobufs.
func errUnsupportedMergeType(a *internalValue) error {
	return spannerErrorf(codes.FailedPrecondition, "unsupported type merge (%s)", internalValueKindName(a))
}

// merge tries to combine two protobuf Values if possible.
func (p *partialResultSetDecoder) merge(a, b *internalValue) (*internalValue, error) {
	var err error
	typeErr := errIncompatibleMergeTypes(a, b)
	switch internalValueKindOf(a) {
	case internalValueString:
		if internalValueKindOf(b) != internalValueString {
			return nil, typeErr
		}
		return internalNewStringValue(a.GetStringValue() + b.GetStringValue()), nil
	case internalValueList:
		if internalValueKindOf(b) != internalValueList {
			return nil, typeErr
		}
		av := internalListValues(a.GetListValue())
		bv := internalListValues(b.GetListValue())
		if len(bv) == 0 {
		...
		if la := len(av) - 1; p.isMergeable(av[la]) {
			...
			av[la], err = p.merge(av[la], bv[0])
			if err != nil {
				return nil, err
			}
			bv = bv[1:]
		}
		return internalNewListValue(append(av, bv...)), nil
	default:
