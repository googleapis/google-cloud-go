...
// The minimum number of split points needed for the range splitter.
const (
	minimumSplits = 2
)
...
func (rs *rangeSplitter) splitRange(startRange, endRange string, numSplits int) ([]string, error) {
	...
	if numSplits < 1 {
		return nil, fmt.Errorf("number of splits should be at least %d, got %d", minimumSplits, numSplits)
	}
