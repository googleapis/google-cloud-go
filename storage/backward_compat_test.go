...
// the legacy signature func(error) bool and WithErrorFuncWithContext accepts
// the new signature func(error, *RetryContext) bool.
func TestWithErrorFuncBackwardCompatibility(t *testing.T) {
	...
	t.Run("new_signature_func(error,_*RetryContext)_bool", func(t *testing.T) {
		...
		// Apply the new function with the new API
		opt := WithErrorFuncWithContext(newFunc)
		rc := &retryConfig{}
		opt.apply(rc)
