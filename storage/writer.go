...
type Writer struct {
	...
	// per-chunk and for the full object. To disable this behavior, see DisableAutoChecksum.
	//
	...
	// populated, that checksum will be sent to GCS for validation.
	//
	// Note: DisableAutoChecksum must be set before the first call to
	// Writer.Write(). It does not work with appendable writes for unfinalized
	// objects.
