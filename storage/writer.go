...
type Writer struct {
	...
	// an error but may leave data on the server. This persistence issue does not apply
	// to user-provided checksums. Callers relying on auto-checksum should handle the
	...
	// Writer.Write(). Automatic checksumming is not enabled full object checksums
	// for unfinalized writes to appendable objects in gRPC.
