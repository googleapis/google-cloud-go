...
var newRegionMultiMigMembersClientHook clientHook
...
func defaultRegionMultiMigMembersRESTCallOptions() *RegionMultiMigMembersCallOptions {
	return &RegionMultiMigMembersCallOptions{
		Get: []gax.CallOption{
...
type internalRegionMultiMigMembersClient interface {
	...
	Get(context.Context, *computepb.GetRegionMultiMigMemberRequest, ...gax.CallOption) (*computepb.MultiMigMember, error)
...
type RegionMultiMigMembersClient struct {
	...
	internalClient internalRegionMultiMigMembersClient
...
func (c *RegionMultiMigMembersClient) setGoogleClientInfo(keyval ...string) {
...
// Methods, except Close, may be called concurrently. However, fields must not be modified concurrently with method calls.
type regionMultiMigMembersRESTClient struct {
	...
	// Points back to the CallOptions field of the containing RegionMultiMigMembersClient
	CallOptions **RegionMultiMigMembersCallOptions
...
func NewRegionMultiMigMembersRESTClient(ctx context.Context, opts ...option.ClientOption) (*RegionMultiMigMembersClient, error) {
	clientOpts := append(defaultRegionMultiMigMembersRESTClientOptions(), opts...)
	...
	callOpts := defaultRegionMultiMigMembersRESTCallOptions()
	c := &regionMultiMigMembersRESTClient{
		endpoint:    endpoint,
	...
	return &RegionMultiMigMembersClient{internalClient: c, CallOptions: callOpts}, nil
}

func defaultRegionMultiMigMembersRESTClientOptions() []option.ClientOption {
...
// the `x-goog-api-client` header passed on each request. Intended for
// use by Google-written clients.
func (c *regionMultiMigMembersRESTClient) setGoogleClientInfo(keyval ...string) {
...
// Close closes the connection to the API service. The user should invoke this when
// the client is no longer required.
func (c *regionMultiMigMembersRESTClient) Close() error {
...
func (c *regionMultiMigMembersRESTClient) Connection() *grpc.ClientConn {
	return nil
}

// Get retrieves information about the specified multi-MIG member.
func (c *regionMultiMigMembersRESTClient) Get(ctx context.Context, req *computepb.GetRegionMultiMigMemberRequest, opts ...gax.CallOption) (*computepb.MultiMigMember, error) {
	baseUrl, err := url.Parse(c.endpoint)
	if err != nil {
		return nil, err
	}
	baseUrl.Path += fmt.Sprintf("/compute/beta/projects/%v/regions/%v/multiMigs/%v/multiMigMembers/%v", req.GetProject(), req.GetRegion(), req.GetMultiMig(), req.GetMultiMigMember())

	// Build HTTP headers from client and context metadata.
	hds := []string{"x-goog-request-params", fmt.Sprintf("%s=%v&%s=%v&%s=%v&%s=%v", "project", url.QueryEscape(req.GetProject()), "region", url.QueryEscape(req.GetRegion()), "multi_mig", url.QueryEscape(req.GetMultiMig()), "multi_mig_member", url.QueryEscape(req.GetMultiMigMember()))}

	hds = append(c.xGoogHeaders, hds...)
	hds = append(hds, "Content-Type", "application/json")
	headers := gax.BuildHeaders(ctx, hds...)
	if gax.IsFeatureEnabled("TRACING") || gax.IsFeatureEnabled("LOGGING") {
		ctx = callctx.WithTelemetryContext(ctx, "resource_name", fmt.Sprintf("//compute.googleapis.com//compute/beta/projects/%v/regions/%v/multiMigs/%v/multiMigMembers/%v", req.GetProject(), req.GetRegion(), req.GetMultiMig(), req.GetMultiMigMember()))
	}
	if gax.IsFeatureEnabled("METRICS") || gax.IsFeatureEnabled("TRACING") || gax.IsFeatureEnabled("LOGGING") {
		ctx = callctx.WithTelemetryContext(ctx, "rpc_method", "google.cloud.compute.v1beta.RegionMultiMigMembers/Get")
		ctx = callctx.WithTelemetryContext(ctx, "url_template", "/compute/beta/projects/{project}/regions/{region}/multiMigs/{multi_mig}/multiMigMembers/{multi_mig_member}")
	}
	opts = append((*c.CallOptions).Get[0:len((*c.CallOptions).Get):len((*c.CallOptions).Get)], opts...)
	unm := protojson.UnmarshalOptions{AllowPartial: true, DiscardUnknown: true}
	resp := &computepb.MultiMigMember{}
...
func (c *regionMultiMigMembersRESTClient) List(ctx context.Context, req *computepb.ListRegionMultiMigMembersRequest, opts ...gax.CallOption) *MultiMigMemberIterator {
	...
	it.InternalFetch = func(pageSize int, pageToken string) ([]*computepb.MultiMigMember, string, error) {
		...
		baseUrl.Path += fmt.Sprintf("/compute/beta/projects/%v/regions/%v/multiMigs/%v/multiMigMembers", req.GetProject(), req.GetRegion(), req.GetMultiMig())
