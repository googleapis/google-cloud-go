...
# Tests in https://github.com/googleapis/cloud-bigtable-clients-test/tree/main/tests can only be run on go1.24.0
go install golang.org/dl/go1.24.0@latest
go1.24.0 download
# known_failures.txt contains tests for the unimplemented features
eval "go1.24.0 test -v -proxy_addr=:$testProxyPort -skip `tr -d '\n' < $testProxyHome/known_failures.txt` | tee -a $sponge_log"
RETURN_CODE=$?
