...
- **`session` is a `*sessionTableHandle` that self-heals across cache eviction.** TableShim caches the handle pointer at Open time and holds it for the shim's lifetime. When the `sessionTableCache` TTL sweeper evicts that handle (or any owner calls `handle.Close()`), the handle's `evicted atomic.Bool` flips to true BEFORE its underlying api is torn down; the next `handle.ReadRow`/`MutateRow` observes the flag on its atomic-Load fast path and routes through `cache.getOrOpen` to fetch (or mint) a live successor, transparently dispatching the RPC there. TableShim never sees the eviction — its cached pointer stays valid forever, and per-RPC cost stays at one atomic Load on the happy path. See `session_table_cache.go` for the invariants; guarded by `TestSessionTableHandle_EvictedSelfHeals`.
