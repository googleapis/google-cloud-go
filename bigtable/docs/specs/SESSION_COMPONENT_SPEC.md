...
| PeakEwma tracker | `PeakEwma` — `peak_ewma.go:24` | `SessionList.PeakEwma` — `SessionList.java:415` |
...
| Per-attempt `server_latencies` (session path) | `sessionTable.stampAttempt` sets `0` unconditionally (`internal/session/table.go`) | `InvokeResult.Stats.BackendLatency` is deliberately unused — it's not wire-comparable to classic's server-timing value; session AFE overhead lives in `transport_latencies` separately. The `connectivity_error_count` classifier at `internal/metrics/tracer.go` gates on `transportType != ""` (PeerInfo prong) so a zero on session path does not misclassify — see `CLIENT_SIDE_METRICS_SPEC.md #2` "server_latencies" + interlock bullets. |
