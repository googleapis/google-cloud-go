...
func setCellWithOrigin(
	tableName string, ts int64, origin btpb.Mutation_TimestampOrigin,
) *btpb.MutateRowRequest {
	req := setCell(tableName, ts)
	req.Mutations[0].TimestampOrigin = origin
	return req
}
...
// TestClientAutoGeneratedTimestampIsTruncated checks the timestamp_origin
// contract from google/bigtable/v2/data.proto: the server truncates a
// CLIENT_AUTO_GENERATED timestamp to the table's granularity, and rejects a
// USER_SPECIFIED one whose precision does not match.
func TestClientAutoGeneratedTimestampIsTruncated(t *testing.T) {
	ctx := context.Background()
	for _, tc := range []struct {
		desc        string
		granularity btapb.Table_TimestampGranularity
		origin      btpb.Mutation_TimestampOrigin
		ts          int64
		wantErr     bool
		wantStored  int64
	}{
		{
			desc:        "millis table truncates a client-generated micros timestamp",
			granularity: btapb.Table_MILLIS,
			origin:      btpb.Mutation_CLIENT_AUTO_GENERATED,
			ts:          2001,
			wantStored:  2000,
		},
		{
			desc:        "millis table rejects a user-specified micros timestamp",
			granularity: btapb.Table_MILLIS,
			origin:      btpb.Mutation_USER_SPECIFIED,
			ts:          2001,
			wantErr:     true,
		},
		{
			desc:        "an unset origin keeps rejecting, as before",
			granularity: btapb.Table_MILLIS,
			origin:      btpb.Mutation_TIMESTAMP_ORIGIN_UNSPECIFIED,
			ts:          2001,
			wantErr:     true,
		},
		{
			desc:        "micros table keeps the precision it can store",
			granularity: btapb.Table_MICROS,
			origin:      btpb.Mutation_CLIENT_AUTO_GENERATED,
			ts:          2001,
			wantStored:  2001,
		},
		{
			desc:        "an aligned client-generated timestamp is unchanged",
			granularity: btapb.Table_MILLIS,
			origin:      btpb.Mutation_CLIENT_AUTO_GENERATED,
			ts:          2000,
			wantStored:  2000,
		...
		t.Run(tc.desc, func(t *testing.T) {
			s, name := newGranularityServer(t, tc.granularity)
			_, err := s.MutateRow(ctx, setCellWithOrigin(name, tc.ts, tc.origin))
			if gotErr := err != nil; gotErr != tc.wantErr {
				t.Fatalf("MutateRow(ts=%d, origin=%v) error = %v, wantErr = %v",
					tc.ts, tc.origin, err, tc.wantErr)
			}
			if tc.wantErr {
				if got := status.Code(err); got != codes.InvalidArgument {
					t.Errorf("MutateRow error code = %v, want %v", got, codes.InvalidArgument)
			...
			if got := readCellTimestamps(t, s, name); len(got) != 1 || got[0] != tc.wantStored {
				t.Errorf("stored timestamps = %v, want [%d]", got, tc.wantStored)
			}
		})
	}
}
