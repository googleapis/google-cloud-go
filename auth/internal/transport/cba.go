...
type Options struct {
	...
	ClientCertProvider  cert.Source
	Client              *http.Client
}
...
func GetHTTPTransportConfig(opts *Options) (cert.Source, func(context.Context, string, string) (net.Conn, error), error) {
	config, err := getTransportConfig(opts)
	if err != nil {
		return nil, nil, err
	}
...
func getClientCertificateSource(opts *Options) (cert.Source, error) {
...
func getEndpoint(opts *Options, clientCertSource cert.Source) (string, error) {
	if opts.Endpoint == "" {
...
func shouldUseS2A(clientCertSource cert.Source, opts *Options) bool {
