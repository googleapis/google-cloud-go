...
type TrustConfigIterator struct {
	items    []*certificatemanagerpb.TrustConfig
	pageInfo *iterator.PageInfo
	nextFunc func() error
...
func (it *TrustConfigIterator) Next() (*certificatemanagerpb.TrustConfig, error) {
	var item *certificatemanagerpb.TrustConfig
	if err := it.nextFunc(); err != nil {
		return item, err
	}
	item = it.items[0]
...
func (it *TrustConfigIterator) bufLen() int {
	return len(it.items)
}

func (it *TrustConfigIterator) takeBuf() interface{} {
	b := it.items
	it.items = nil
	return b
}
