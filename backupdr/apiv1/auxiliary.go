...
type ResourceBackupConfigIterator struct {
	items    []*backupdrpb.ResourceBackupConfig
	pageInfo *iterator.PageInfo
	nextFunc func() error
	...
	// The number of results is no greater than pageSize.
	// If there are no more results, nextPageToken is empty and err is nil.
	InternalFetch func(pageSize int, pageToken string) (results []*backupdrpb.ResourceBackupConfig, nextPageToken string, err error)
}

// PageInfo supports pagination. See the [google.golang.org/api/iterator] package for details.
func (it *ResourceBackupConfigIterator) PageInfo() *iterator.PageInfo {
	return it.pageInfo
}

// Next returns the next result. Its second return value is iterator.Done if there are no more
// results. Once Next returns Done, all subsequent calls will return Done.
func (it *ResourceBackupConfigIterator) Next() (*backupdrpb.ResourceBackupConfig, error) {
	var item *backupdrpb.ResourceBackupConfig
	if err := it.nextFunc(); err != nil {
		return item, err
	}
	item = it.items[0]
...
func (it *ResourceBackupConfigIterator) bufLen() int {
	return len(it.items)
}

func (it *ResourceBackupConfigIterator) takeBuf() interface{} {
	b := it.items
	it.items = nil
	return b
}
