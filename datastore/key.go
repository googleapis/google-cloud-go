...
func NewIncompleteKey(c Client, kind string, parent *Key) *Key {
...
func NewKey(c Client, kind, name string, id int64, parent *Key) *Key {
	return &Key{
		kind:      kind,
		name:      name,
		id:        id,
...
func AllocateIDs(c Client, keys []*Key) ([]*Key, error) {
	if keys == nil {
		return nil, nil
	}
