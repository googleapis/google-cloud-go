...
// PutItem represents a single entity's data along with any options for its
// mutation within a commit operation. This is used by PutWithOptions and PutMultiWithOptions.
type PutItem struct {
	Key        *Key
	Entity     interface{}
...
func (c *Client) Put(ctx context.Context, key *Key, src interface{}) (*Key, error) {
	return c.PutWithOptions(ctx, &PutItem{Key: key, Entity: src})
}

// PutWithOptions saves a single entity, applying the options defined in the PutItem.
func (c *Client) PutWithOptions(ctx context.Context, item *PutItem) (*Key, error) {
	k, err := c.PutMultiWithOptions(ctx, []*PutItem{item})
	if err != nil {
		if me, ok := err.(MultiError); ok {
			return nil, me[0]
		}
...
func (c *Client) PutMulti(ctx context.Context, keys []*Key, src interface{}) (ret []*Key, err error) {
	return c.putMultiInternal(ctx, keys, nil, src, false)
}

// PutMultiWithOptions is a batch version of PutWithOptions.
func (c *Client) PutMultiWithOptions(ctx context.Context, items []*PutItem) (ret []*Key, err error) {
	var keys []*Key
	...
	for _, item := range items {
		...
		transforms = append(transforms, item.Transforms)
		srcs = append(srcs, item.Entity)
	}
	return c.putMultiInternal(ctx, keys, transforms, srcs, true)
}

func (c *Client) putMultiInternal(ctx context.Context, keys []*Key, transforms [][]PropertyTransform, src any, hasTransforms bool) (ret []*Key, err error) {
	// TODO(jba): rewrite in terms of Mutate.
	ctx = trace.StartSpan(ctx, "cloud.google.com/go/datastore.PutMulti")
	defer func() { trace.EndSpan(ctx, err) }()

	mutations, err := putMutations(keys, src, transforms, hasTransforms)
	if err != nil {
		return nil, err
	}
...
func putMutations(keys []*Key, src interface{}, transforms [][]PropertyTransform, hasTransforms bool) ([]*pb.Mutation, error) {
	...
	for i, k := range keys {
		...
		var pbTranforms []*pb.PropertyTransform
		if hasTransforms {
			for _, t := range transforms[i] {
				pbTranforms = append(pbTranforms, t.pb)
			}
		}
		var mut *pb.Mutation
		if k.Incomplete() {
			mut = &pb.Mutation{Operation: &pb.Mutation_Insert{Insert: p}, PropertyTransforms: pbTranforms}
		} else {
			mut = &pb.Mutation{Operation: &pb.Mutation_Upsert{Upsert: p}, PropertyTransforms: pbTranforms}
		}
