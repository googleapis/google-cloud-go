...
// Next returns the next result. Its second return value is iterator.Done if there are no more
// results. Once Next returns Done, all subsequent calls will return Done.
func (it *McpServerIterator) Next() (*apiregistrypb.McpServer, error) {
	var item *apiregistrypb.McpServer
	if err := it.nextFunc(); err != nil {
		return item, err
	}
	item = it.items[0]
...
// PageInfo supports pagination. See the [google.golang.org/api/iterator] package for details.
func (it *McpToolIterator) PageInfo() *iterator.PageInfo {
	return it.pageInfo
}

// Next returns the next result. Its second return value is iterator.Done if there are no more
// results. Once Next returns Done, all subsequent calls will return Done.
func (it *McpToolIterator) Next() (*apiregistrypb.McpTool, error) {
	var item *apiregistrypb.McpTool
	if err := it.nextFunc(); err != nil {
		return item, err
	}
	item = it.items[0]
...
func (it *McpToolIterator) bufLen() int {
	return len(it.items)
}

func (it *McpToolIterator) takeBuf() interface{} {
	b := it.items
	it.items = nil
	return b
}
