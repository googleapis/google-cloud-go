...
// New calls NewSep with '-' as the separator rune.
func (s *UIDSpace) New() string { return s.NewSep('-') }
...
// and sep.
func (s *UIDSpace) NewSep(sep rune) string { return s.newID(startTime, sep) }

func (s *UIDSpace) newID(t time.Time, sep rune) string {
	s.mu.Lock()
	c := s.count
	s.count++
	s.mu.Unlock()
	...
	return fmt.Sprintf("%s%c%04d%02d%02d%c%d%c%04d", s.Prefix, sep, y, m, d, sep, ns, sep, c)
}
