...
func ParseChangeInfo(googleapisDir string, hashes []string) ([]*ChangeInfo, error) {
	...
	for _, hash := range hashes {
		...
		c := command.Create("git", "show", "--pretty=format:%s~~%b", "-s", hash)
		c.Stdout = rawBody
		c.Dir = googleapisDir
...
func CommitsSinceHash(gitDir, hash string, inclusive bool) ([]string, error) {
	...
	c := command.Create("git", "rev-list", commitRange)
	c.Stdout = out
	c.Dir = gitDir
	if err := c.Run(); err != nil {
		return nil, err
...
func UpdateFilesSinceHash(gitDir, hash string) ([]string, error) {
	...
	// - (M) Modified
	// - (R) Renamed
	c := command.Create("git", "diff-tree", "--no-commit-id", "--name-only", "--diff-filter=ACMR", "-r", fmt.Sprintf("%s..HEAD", hash))
	c.Stdout = out
	c.Dir = gitDir
	if err := c.Run(); err != nil {
		return nil, err
...
func filesChanged(gitDir, hash string) ([]string, error) {
	out := bytes.NewBuffer(nil)
	c := command.Create("git", "show", "--pretty=format:", "--name-only", hash)
	c.Stdout = out
	c.Dir = gitDir
	if err := c.Run(); err != nil {
		return nil, err
