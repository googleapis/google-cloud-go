...
var (
	...
	subscriptionID = flag.String("subscription", "", "Google Cloud Platform PubSub subscription ID.")
)
...
func main() {
	...
	if *subscriptionID == "" {
		fmt.Fprintln(os.Stderr, "-subscription flag is required")
		flag.Usage()
		os.Exit(2)
	}
	...
	// adjust the ack behavior below to batch ack
	...
	for {
		...
		go func() {
			for {
				...
				t := time.NewTimer(ackDeadline - 5*time.Second)
				select {
				case <-ctx.Done():
					t.Stop()
					return
		...
		for _, m := range res.ReceivedMessages {
			...
			fmt.Println("Got message,", string(m.Message.Data))
