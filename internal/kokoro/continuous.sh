...
# curl -L -O https://github.com/GoogleCloudPlatform/grpc-gcp-tools/releases/download/v1.11/dp_check
# chmod +x dp_check
...
runDirectoryTests() {
  ...
  GRPC_GO_LOG_VERBOSITY_LEVEL=99 GRPC_GO_LOG_SEVERITY_LEVEL=info GOWORK=off go test -race -v -timeout 45m "${1:-./...}" 2>&1 |
    tee sponge_log.log
