...
func (c *SubscriberClient) StreamingPull(ctx context.Context, opts ...gax.CallOption) (pubsubpb.Subscriber_StreamingPullClient, error) {
	...
	if _, set := ctx.Deadline(); !set {
		ctx, cancel = context.WithTimeout(ctx, 900000*time.Millisecond)
	}
	ctx = insertMetadata(ctx, c.xGoogMetadata)
	opts = append(c.CallOptions.StreamingPull[0:len(c.CallOptions.StreamingPull):len(c.CallOptions.StreamingPull)], opts...)
	var resp *subscriberStreamingPullClient
	err := gax.Invoke(ctx, func(ctx context.Context, settings gax.CallSettings) error {
		stub, err := c.subscriberClient.StreamingPull(ctx, settings.GRPC...)
		if err != nil {
			return err
		}
		resp = &subscriberStreamingPullClient{
			Subscriber_StreamingPullClient: stub,
			cancel:                         cancel,
		}
		return nil
	}, opts...)
...
func (x *subscriberStreamingPullClient) Recv() (*pubsubpb.StreamingPullResponse, error) {
	res, err := x.Subscriber_StreamingPullClient.Recv()
	if x.cancel != nil {
		x.mu.Lock()
		defer x.mu.Unlock()
...
func (x *subscriberStreamingPullClient) Send(m *pubsubpb.StreamingPullRequest) error {
	err := x.Subscriber_StreamingPullClient.Send(m)
...
func (x *subscriberStreamingPullClient) CloseSend() error {
	err := x.Subscriber_StreamingPullClient.CloseSend()
	if x.cancel != nil {
		x.mu.Lock()
		defer x.mu.Unlock()
		if err != nil || x.sDone {
